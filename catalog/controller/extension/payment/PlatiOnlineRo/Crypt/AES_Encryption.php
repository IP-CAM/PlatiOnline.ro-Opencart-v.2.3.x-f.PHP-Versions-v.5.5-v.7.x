 ...
 *
 *  Modified by EuFlo in may 2020
 *  added OpenSSL library option
 *
 **/
...
class AES_Encryption
{
	...
	{
		//$this->logaes = new Log('LOG_AES_Ecryption.log');
		...
		if (!function_exists('mcrypt_module_open')) {
			...
			$this->use_openssl = true;
			//$this->logaes->write('DEBUG:[AES_Encryption]: use_openssl=true');
		} else {

			//$this->logaes->write('DEBUG:[AES_Encryption]: use mycrypt');
		}
		...
		if (!$this->use_openssl)  // Modified by EuFlo
		{
			// mcrypt
	...
	public function encrypt($text)
	{
		...
		} else { // Modified by EuFlo
			//Without using OPENSSL_ZERO_PADDING, you will automatically get PKCS#7 padding
			if ($this->padding == 'ZERO') {
				$encrypted_text = openssl_encrypt($text, 'AES-256-CBC', $this->key, OPENSSL_RAW_DATA | OPENSSL_ZERO_PADDING, $this->initVector);
			} else {
				$encrypted_text = openssl_encrypt($text, 'AES-256-CBC', $this->key, OPENSSL_RAW_DATA, $this->initVector);
			}


			if ($encrypted_text == false) {
				//$this->logaes->write('DEBUG:[AES_Encryption]: ERROR: openssl_ecrypyt() returned false');
				throw new Exception('Error:AES_Encryption, openssl_ecrypyt() returned false');
			}
	...
	public function decrypt($text)
	{
		if (!$this->use_openssl) {
			...
			if ($this->padding == 'ZERO') {
				$decrypted_text = openssl_decrypt($text, 'AES-' . $this->encryption . '-CBC', $this->key, OPENSSL_ZERO_PADDING, $this->initVector);
			} else {
				$decrypted_text = openssl_decrypt($text, 'AES-' . $this->encryption . '-CBC', $this->key, OPENSSL_RAW_DATA, $this->initVector);
			}


			//$this->logaes->write("DEBUG:[AES_Encryption]: key length=".$this->encryption);

			if ($decrypted_text == false) {

				throw new Exception('Error:AES_Encryption, openssl_decrypt() returned false');
			}
