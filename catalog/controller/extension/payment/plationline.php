...
class ControllerExtensionPaymentPlatiOnline extends Controller {
	public function index() {
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/plationline.tpl')) {
			...
			return $this->load->view('extension/payment/plationline.tpl', $data);
		}
	}

	public function callback() {
		$this->language->load('extension/payment/plationline');
		require_once("catalog/controller/extension/payment/PlatiOnlineRo/po5.php");

		$po = new PO5();
		$po->f_login = $this->config->get("plationline_f_login");
		$po->setRSAKeyDecrypt($this->config->get("plationline_rsa_itsn"));
		$po->setIVITSN($this->config->get("plationline_iv_itsn"));

		$po_f_relay_method = $this->config->get("plationline_relay_method");
			...
			case 'PTOR':
				...
				switch($X_RESPONSE_CODE) {
					case '2':
						...
						$this->model_checkout_order->addOrderHistory($order_id, $this->config->get('plationline_order_status_id'), 'PlatiOnline.ro Tranzaction ID: <strong>' . $trans_id . '</strong>',TRUE);
						// Stock subtraction
					...
					case '17':
						//	eroare
						$order_info = $this->model_checkout_order->getOrder($order_id);
						if ($order_info['order_status_id'] != $this->config->get('plationline_order_status_id')) {
	...
	public function itsn() {
		...
		$po->setRSAKeyDecrypt($this->config->get("plationline_rsa_itsn"));
		$po->setIVITSN($this->config->get("plationline_iv_itsn"));
		$po_itsn_method = $this->config->get("plationline_itsn_method");

		switch ($po_itsn_method) {
			case 'POST':
				$call_itsn = $po->itsn($_POST['f_itsn_message'], $_POST['f_crypt_message']);
				$po->f_login = $this->config->get("plationline_f_login");
				$po->setRSAKeyEncrypt($this->config->get("plationline_rsa_auth"));
				$po->setIV($this->config->get("plationline_iv_auth"));
				...
				if ($po->get_xml_tag_content($raspuns_itsn,'PO_ERROR_CODE') == 1) {
					...
					switch ($starefin1) {
						case '13':
							...
							$this->model_checkout_order->addOrderHistory($order_id, $this->config->get('plationline_order_status_on_hold_id'), 'PlatiOnline.ro Tranzaction ID Status Notification: <strong>' . $trans_id . '</strong>', TRUE);
							break;
						case '2':
							//$starefin = 'Autorizata';
							$order_info = $this->model_checkout_order->getOrder($order_id);
							if ($order_info['order_status_id'] != $this->config->get('plationline_order_status_id')) {
								$this->model_checkout_order->addOrderHistory($order_id, $this->config->get('plationline_order_status_id'), 'PlatiOnline.ro Tranzaction ID Status Notification: <strong>' . $trans_id . '</strong>', TRUE);
								// Stock subtraction
								$order_product_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_product WHERE order_id = '" . (int)$order_id . "'");
						...
						case '8':
							...
							$order_info = $this->model_checkout_order->getOrder($order_id);
							if ($order_info['order_status_id'] != $this->config->get('plationline_order_status_id')) {
								$this->model_checkout_order->addOrderHistory($order_id, $this->config->get('plationline_order_status_decline_id'), 'Tranzactia a fost refuzata. PlatiOnline.ro Tranzaction ID Status Notification: <strong>' . $trans_id . '</strong>', TRUE);
							}
							break;
						case '3':
							//$starefin = 'In curs de incasare';
							$this->model_checkout_order->addOrderHistory($order_id, $this->config->get('plationline_order_status_pending_settled_id'), 'Tranzactie in curs de incasare. PlatiOnline.ro Tranzaction ID Status Notification: <strong>' . $trans_id . '</strong>', TRUE);
							break;
						case '5':
							//$starefin = 'Incasata';
							...
							switch ($starefin2){
								case '1':
									...
									$this->model_checkout_order->addOrderHistory($order_id, $this->config->get('plationline_order_status_pending_credited_id'), 'Tranzactie in curs de creditare. PlatiOnline.ro Tranzaction ID Status Notification: <strong>' . $trans_id . '</strong>', TRUE);
									break;
								case '2':
									//$starefin='Creditata';
									$this->model_checkout_order->addOrderHistory($order_id, $this->config->get('plationline_order_status_credited_id'), 'Suma a fost creditata inapoi pe cardul clientului. PlatiOnline.ro Tranzaction ID Status Notification: <strong>' . $trans_id . '</strong>', TRUE);
									break;
								case '3':
									//$starefin='Refuz la plata';
									$this->model_checkout_order->addOrderHistory($order_id, $this->config->get('plationline_order_status_cbk_id'), 'Suma a fost creditata inapoi pe cardul clientului conform refuzului de plata al acestuia. PlatiOnline.ro Tranzaction ID Status Notification: <strong>' . $trans_id . '</strong>', TRUE);
									break;
								case '4':
									//$starefin='Incasata';
									$this->model_checkout_order->addOrderHistory($order_id, $this->config->get('plationline_order_status_settled_id'), 'Suma a fost incasata de pe cardul clientului. PlatiOnline.ro Tranzaction ID Status Notification: <strong>' . $trans_id . '</strong>', TRUE);
									break;
							}
							break;
						case '6':
							//$starefin= 'In curs de anulare';
							$this->model_checkout_order->addOrderHistory($order_id, $this->config->get('plationline_order_status_pending_voided_id'), 'In curs de anulare. PlatiOnline.ro Tranzaction ID Status Notification: <strong>' . $trans_id . '</strong>', TRUE);
							break;
						case '7':
							//$starefin='Anulata';
							$this->model_checkout_order->addOrderHistory($order_id, $this->config->get('plationline_order_status_voided_id'), 'Suma a fost deblocata si este disponibila pe cardul clientului. PlatiOnline.ro Tranzaction ID Status Notification: <strong>' . $trans_id . '</strong>', TRUE);
							break;
						case '9':
							//$starefin='Expirata 30 zile';
							$this->model_checkout_order->addOrderHistory($order_id, $this->config->get('plationline_order_status_expired_id'), 'Tranzactie expirata 30 zile. PlatiOnline.ro Tranzaction ID Status Notification: <strong>' . $trans_id . '</strong>', TRUE);
							break;
						case '10':
						case '16':
						case '17':
							...
							$order_info = $this->model_checkout_order->getOrder($order_id);
							if ($order_info['order_status_id'] != $this->config->get('plationline_order_status_id')) {
								$this->model_checkout_order->addOrderHistory($order_id, $this->config->get('plationline_order_status_error_id'), 'Eroare autorizare. PlatiOnline.ro Tranzaction ID Status Notification: <strong>' . $trans_id . '</strong>', TRUE);
							}
							break;
						case '1':
							//$starefin='In curs de autorizare';
							$this->model_checkout_order->addOrderHistory($order_id, $this->config->get('plationline_order_status_pending_id'), 'In curs de autorizare. PlatiOnline.ro Tranzaction ID Status Notification: <strong>' . $trans_id . '</strong>', TRUE);
							break;
						default:
	...
	public function confirm() {
		...
		if ($order_info['payment_telephone'] || $order_info['telephone']) {
			$customer_info['contact']['f_phone'] = isset($order_info['payment_telephone'])?$order_info['payment_telephone']:$order_info['telephone'];
		}
		if ($order_info['payment_telephone'] || $order_info['telephone']) {
			$customer_info['contact']['f_mobile_number'] = isset($order_info['payment_telephone'])?$order_info['payment_telephone']:$order_info['telephone'];
		}
		...
		if ($this->cart->hasShipping()) {
			...
			$shipping_info['contact']['f_phone'] 		 = isset($order_info['shipping_telephone'])?$order_info['payment_telephone']:$order_info['telephone'];
			$shipping_info['contact']['f_mobile_number'] = isset($order_info['shipping_telephone'])?$order_info['payment_telephone']:$order_info['telephone'];
		...
		$transaction_relay_response['f_relay_method']		= $this->config->get('plationline_relay_method');
		...
		foreach ( $this->cart->getProducts() as $product ) {
			...
			$item_without_vat = $product['price'] - $item_vat;
		...
		if ($this->cart->hasShipping()) {
			if (isset($this->session->data['shipping_method']["cost"])) {
				if ($this->tax->getTax($this->session->data['shipping_method']["cost"], $this->session->data['shipping_method']['tax_class_id']) !== false) {
					...
					$shipping_without_vat = $this->session->data['shipping_method']["cost"] - $shipping_vat;
				}
			} else {
				$shipping_vat = 0;
		...
		$po->setRSAKeyEncrypt($this->config->get("plationline_rsa_auth"));
		$po->setIV($this->config->get("plationline_iv_auth"));
		$po->test_mode = $this->config->get("plationline_test");
